...
ToolMain::ToolMain()
{
	...
	m_selectedObjectID = -1;	//initial selection ID
...
{
	if (m_d3dRenderer.DeselectRequired())
	{
		m_selectedObjectID = -1;
		m_d3dRenderer.DeselectPerformed();
	}

	if(m_d3dRenderer.GetSelectionID() != m_selectedObjectID)
		m_d3dRenderer.SetSelectionID(m_selectedObjectID);
...
void ToolMain::onActionInitialise(HWND handle, HWND mainHandle, int width, int height)
{
	...
	m_objectLoader = new ObjectLoader(m_sceneGraph, m_d3dRenderer.GetDisplayList(), &m_d3dRenderer.GetDevice(), &m_d3dRenderer.GetEffectFactory());
	
	m_d3dRenderer.LoadObjects(m_objectLoader);
}
...
void ToolMain::Tick(MSG *msg)
{
	...
	{
		m_selectedObjectID = m_d3dRenderer.MousePicking(true);
		//m_toolInputCommands.mouseLBDown = false;
	}
	else if(m_d3dRenderer.GetTransformStarted() && !m_toolInputCommands.mouseRBDown)
		m_d3dRenderer.SetTransformEnd();

	if (m_toolInputCommands.mouseRBDown && GetForegroundWindow() == m_mainHandle)
	{
		m_selectedObjectID = m_d3dRenderer.MousePicking();
	}
	else if (m_d3dRenderer.GetTransformStarted() && !m_toolInputCommands.mouseLBDown)
		m_d3dRenderer.SetTransformEnd();
	...
	if (m_objectLoader->UpdateRequired() || m_d3dRenderer.DisplayListUpdateRequired())
	{
		//m_sceneGraph = m_d3dRenderer.GetSceneGraph();
		m_d3dRenderer.BuildDisplayList(&m_sceneGraph);
		m_objectLoader->SetUpdateRequired(false);
		m_d3dRenderer.SetDisplayListUpdateRequired(false);
	}
...
void ToolMain::UpdateInput(MSG * msg)
{
	...
	//Movement
	m_keyArray['W'] ? m_toolInputCommands.forward = true : m_toolInputCommands.forward = false;
	m_keyArray['A'] ? m_toolInputCommands.left = true : m_toolInputCommands.left = false;
	m_keyArray['S'] ? m_toolInputCommands.back = true : m_toolInputCommands.back = false;
	m_keyArray['D'] ? m_toolInputCommands.right = true : m_toolInputCommands.right = false;
	m_keyArray[' '] ? m_toolInputCommands.space = true : m_toolInputCommands.space = false;
	m_keyArray[VK_DOWN] ? m_toolInputCommands.rotateDown = true : m_toolInputCommands.rotateDown = false;
	m_keyArray[VK_UP] ? m_toolInputCommands.rotateUp = true : m_toolInputCommands.rotateUp = false;
	...
	m_keyArray['2'] ? m_toolInputCommands.numTwo = true : m_toolInputCommands.numTwo = false;
	m_keyArray['3'] ? m_toolInputCommands.numThree = true : m_toolInputCommands.numThree = false;
	...
	//Manipulation
	m_keyArray['W'] ? m_toolInputCommands.transform = true : m_toolInputCommands.transform = false;
	m_keyArray['E'] ? m_toolInputCommands.rotate = true : m_toolInputCommands.rotate = false;
	m_keyArray['R'] ? m_toolInputCommands.scale = true : m_toolInputCommands.scale = false;
	m_keyArray[VK_ESCAPE] ? m_toolInputCommands.escape = true : m_toolInputCommands.escape = false;
	m_keyArray[VK_DELETE] ? m_toolInputCommands.del = true : m_toolInputCommands.del = false;
	m_keyArray[VK_SHIFT] ? m_toolInputCommands.shift = true : m_toolInputCommands.shift = false;
	m_keyArray[VK_CONTROL] ? m_toolInputCommands.ctrl = true : m_toolInputCommands.ctrl = false;
}

void ToolMain::DuplicateSelectedObject()
{
	if (m_objectLoader)
	{
		m_d3dRenderer.AddChanges();
		HRESULT result = m_objectLoader->DuplicateSceneObject(m_selectedObjectID);
		if (FAILED(result))
		{
...
void ToolMain::InitInputValues()
{
	...
	//Object Manipulation
	m_toolInputCommands.duplicate = false;
	m_toolInputCommands.transform = false;
